//---------------------------------------------------------------------------
//
// $Id: SWSHSpherical.cc,v 1.1.1.1 2003/08/09 19:19:39 sdrasco Exp $
//
//---------------------------------------------------------------------------
//
...
Real SWSH::pref_A_func(const int l, const int m)
{
  ...
  if (l + m <= 32)
    ans *= sqrt(factrl(l - m)/factrl(l + m));
  else
    ans *= exp(0.5*(gammln(l - m + 1) - gammln(l + m + 1)));
  return ans;
}
...
Real SWSH::plgndr(const int l, const int m, const Real x)
{
  ...
  if (m < 0 || m > l || fabs(x) > 1.)
    Die("Bad arguments in routine plgndr");
...
Real SWSH::plgndr_over_sin(const int l, const int m, const Real x)
{
  ...
  if (l < 1 || m > l || fabs(x) > 1.)
    Die("Bad arguments in routine plgndr_over_sin");
...
Real SWSH::plgndr_over_sinsqr(const int l, const int m, const Real x)
{
  ...
  if (l < 2 || m > l || fabs(x) > 1.)
    Die("Bad arguments in routine plgndr_over_sinsqr");
...
Real SWSH::sin_times_dplgndr(const int l, const int m, const Real x)
{
  ...
  if (l < 1 || m > l || fabs(x) > 1.)
    Die("Bad arguments in routine sin_times_dplgndr");
...
Real SWSH::dplgndr(const int l, const int m, const Real x)
{
  ...
  if (m > l || fabs(x) > 1.)
    Die("Bad arguments in routine dplgndr");
...
Real SWSH::sinsqr_times_ddplgndr(const int l, const int m, const Real x)
{
  ...
  if (l < 2 || m > l || fabs(x) > 1.)
    Die("Bad arguments in routine sinsqr_times_ddplgndr");
...
//
// The below three functions calculate the three varieties of spherical
