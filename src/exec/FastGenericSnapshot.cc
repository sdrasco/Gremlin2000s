...
//
// Currently, this version does not seem to work.
// 
...
// TEST : we might want to use TotalEI and TotalEH
Real TotalEI = 0.0;
Real TotalEH = 0.0;
...
void GetXlmkn()
{
  ...
  IEKR iekr(iekg,l_index,m_index,k_index,n_index,eps_flux,fabs(TotalEI),fabs(TotalEH),DoZH,DoZI);
  Complex ZH = iekr.ZH;
  ...
  // TEST : we might want these total sums
  TotalEI += Xlmkn[0];
  TotalEH += Xlmkn[1];
...
void GetXl(){
  ...
  if(!JUMPSTARTING){
    ...
    // "ContinueSumH_GetXl" and "ContinueSumI_GetXl" will clairty 
    // the subroutines below truncation of the m-sum is ever added.
